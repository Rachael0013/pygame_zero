...
def draw():
    ...
    if not game_over:
        ...
        if say_dance:
            screen.draw.text("DaNcE!", color="black", topleft = (335, 150), fontsize = 60)
        if show_countdown:
            screen.draw.text(str(count), color="black", topleft=(392, 150), fontsize = 60)
    else:
        screen.clear()
        screen.blit("stage", (0,0))
        screen.draw.text("Score: " + str(score), color = "black", topleft = (10,10))
        screen.draw.text("GAME OVER", color = "black", topleft = (270,270), fontsize = 60)
...
def update_dancer(move):
    ...
    if not game_over:
        if move == 0:
            up.image = "up-lit"
            dancer.image = "lisa-up"
            clock.schedule(reset_dancer, 0.5)
        if move == 1:
            right.image = "right-lit"
            dancer.image = "lisa-right"
            clock.schedule(reset_dancer, 0.5)
        if move == 2:
            down.image = "down-lit"
            dancer.image = "lisa-down"
            clock.schedule(reset_dancer, 0.5)
        if move == 3:
            left.image = "left-lit"
            dancer.image = "lisa-left"
            clock.schedule(reset_dancer, 0.5)
...
def generate_moves():
    global move_list, dance_length, count, show_countdown, say_dance, display_list
    count = 4
    move_list = []
    say_dance = False
